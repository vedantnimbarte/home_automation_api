...
class Connection {
  ...
  assignAppliance(
    ...
    return new Promise((resolve, reject) => {
      this.connectionObj.query(
        `INSERT INTO device_allocation_data(serial_no, device_type, user_id, room, appliance, switch, dimmer, relay) VALUES ('24:0A:C4:F9:2A:2C', '${device_type}', '${user}', '${room_name}', '${appliance}', '${switch_status}', '${dimmer_status}', '${relay}')`,
        (error, results, fields) => {
          if (error) {
            console.log(error)
            resolve({ success: 0, message: "Request failed" });
