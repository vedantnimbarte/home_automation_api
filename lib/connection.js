...
class Connection {
  ...
  assignRoomToUser(user, room) {
    return new Promise((resolve, reject) => {
      this.connectionObj.query(
        `INSERT INTO device_allocation_data(serial_no, user_id, room) VALUES('54554546654', '${user}', '${room}')`,
        (error, results, fields) => {
          if (error) {
  ...
  getRoomsByUserId(user) {
    return new Promise((resolve, reject) => {
      this.connectionObj.query(
        `SELECT id, room, serial_no, relay FROM device_allocation_data WHERE user_id = ${user}`,
        (error, results, fields) => {
          if (error) {
            resolve({ success: 0, message: "Request failed" });
          }
  ...
  assignAppliance(
    ...
    return new Promise((resolve, reject) => {
      this.connectionObj.query(
        `INSERT INTO device_allocation_data(serial_no, device_type, user_id, room, appliance, switch, dimmer, relay) VALUES ('54554546654', '${device_type}', '${user}', '${room_name}', '${appliance}', '${switch_status}', '${dimmer_status}', '${relay}')`,
        (error, results, fields) => {
          if (error) {
            console.log(error)
            resolve({ success: 0, message: "Request failed" });
