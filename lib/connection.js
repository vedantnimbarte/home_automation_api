...
class Connection {
  ...
  getRoomsAssignedToUser(room, user) {
    return new Promise((resolve, reject) => {
      this.connectionObj.query(`SELECT * FROM device_allocation_data WHERE room = ${room} AND user_id = ${user}`, (error, results, fields) => {
        if(error) {
          resolve({ success: 0, message: "Request failed" });
        }
        if(results.length > 0) {
          ...
          resolve({ success: 1, message: "No appliances assigned to this room"});
        }
      });
    });
  }
  ...
  getSecurityDevices(user_id) {
    return new Promise((resolve, reject) => {
      this.connectionObj.query(
        `SELECT * FROM device_allocation_data WHERE device_type = 'R1/sec' AND user_id = ${user_id}`,
        (error, results, fields) => {
          if (error) resolve({ success: 0, message: "Request failed" });
          console.log(results)
          if (results.length > 0) {
