...
class Connection {
  ...
  assignAppliance(
    ...
    return new Promise((resolve, reject) => {
      this.connectionObj.query(
        `ALTER TABLE device_allocation_data SET appliance=${appliance}, switch=${switch_status}, dimmer=${dimmer_status}, relay=${relay} WHERE user_id=${user} AND room=${room_name}`
      );
    });
  }
}
